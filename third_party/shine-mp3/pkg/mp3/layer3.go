...
func (enc *Encoder) Write(out io.Writer, data []int16) error {
	...
	channels := int(enc.Wave.Channels)

	samplesRead := len(data)
	chunkSize := samples_per_pass * channels
	for i := 0; i < samplesRead; i += chunkSize {
		...
		outData, written := enc.encodeBufferInterleaved(&chunk[0])
		err := binary.Write(out, binary.LittleEndian, outData[:written])
		if err != nil {
			return err
		}
	}
